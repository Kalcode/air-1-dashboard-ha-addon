...
import { THRESHOLDS, cardStyle, labelStyle, mono } from './thresholds';
...
export const ParticleBreakdown: Component<ParticleBreakdownProps> = (props) => {
  const pm1 = () => Number.parseFloat(props.data.pm_1um) || 0;
  const pm25 = () => Number.parseFloat(props.data.pm25) || 0;
  const pm10 = () => Number.parseFloat(props.data.pm10) || 0;
  ...
  const segments = () => {
    ...
    if (Number.parseFloat(ufPct) > 65) signature = 'Combustion dominant (smoke, fire, candles)';
    else if (Number.parseFloat(cPct) > 40) signature = 'Dust / mechanical (pets, HVAC, construction)';
    else if (Number.parseFloat(fPct) > 35) signature = 'Cooking / mixed combustion';
    ...
    <Show when={segments()}>
      {(seg) => (
        <div style={{ ...cardStyle, 'margin-top': '12px' }}>
          <div style={{ ...labelStyle, 'margin-bottom': '10px' }}>Particle Size Profile</div>
          <div
            style={{
              display: 'flex',
              height: '10px',
              ...
              {(s) => (
                <div
                  style={{
                    width: `${s.pct}%`,
                    background: s.color,
                    'min-width': Number.parseFloat(s.pct) > 0 ? '4px' : '0',
          ...
          <div style={{ display: 'flex', 'justify-content': 'space-between' }}>
            <For each={seg().items}>
              {(s, i) => (
                <div
                  style={{
                    'text-align': i() === 0 ? 'left' : i() === 2 ? 'right' : 'center',
                  }}
                >
                  <span
                    style={{
                  ...
                  <span style={{ color: '#475569', 'font-size': '9px', ...mono }}>{s.value.toFixed(1)} µg/m³</span>
                </div>
              )}
            </For>
          </div>
...
export const WHOBars: Component<WHOBarsProps> = (props) => {
  const items = () => {
    ...
    if (props.data.pm25)
      result.push({
        label: 'PM2.5',
        value: Number.parseFloat(props.data.pm25),
        limit: 15,
        unit: 'µg/m³',
      });
    if (props.data.pm10)
      result.push({
        label: 'PM10',
        value: Number.parseFloat(props.data.pm10),
        limit: 45,
        unit: 'µg/m³',
      });
    return result;
  ...
  return (
    <Show when={items().length > 0}>
      <div style={{ ...cardStyle, 'margin-top': '12px' }}>
        <div style={{ ...labelStyle, 'margin-bottom': '10px' }}>vs WHO Guidelines (24-hr)</div>
        <For each={items()}>
          {(item) => {
            ...
            return (
              <div style={{ 'margin-bottom': '8px' }}>
                ...
                >
                  yours: {item.value.toFixed(1)} · limit: {item.limit} {item.unit}
                </div>
              </div>
            );
          }}
...
export const StatusPanel: Component<StatusPanelProps> = (props) => {
  const computed = () => {
    const tips: Tip[] = [];
    const pm = Number.parseFloat(props.data.pm25);
    const co = Number.parseFloat(props.data.co2);
    const hu = Number.parseFloat(props.data.humidity);
    const vo = Number.parseFloat(props.data.voc);
    ...
    if (!Number.isNaN(vo) && vo >= 150) tips.push({ e: '\uD83D\uDFE0', t: 'VOC index abnormal. Ventilate.' });
    if (props.data.vocQuality === 'Abnormal' || props.data.vocQuality === 'Very Abnormal') {
      tips.push({
        e: '\u26A0\uFE0F',
          ...
          {(tip) => (
            <div
              style={{
                ...
                'border-left': tip.indent ? 'none' : `2px solid ${computed().st.color}33`,
                'line-height': '1.5',
              }}
            >
              {tip.e ? `${tip.e} ` : ''}
