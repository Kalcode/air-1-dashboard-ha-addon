...
export const HistoryCard: Component<HistoryCardProps> = (props) => {
  const pm = () => (props.entry.data.pm25 ? Number.parseFloat(props.entry.data.pm25).toFixed(1) : '\u2014');
  const co = () => (props.entry.data.co2 ? Number.parseFloat(props.entry.data.co2).toFixed(0) : '\u2014');
  ...
  return (
    <div
      style={{
        background: props.isViewing ? '#0c1629' : props.isComparing ? '#0f172a' : '#080e1a',
    ...
    >
      ...
      >
        ...
        <span style={{ 'font-size': '9px', color: '#475569', ...mono }}>{timeAgo(props.entry.timestamp)}</span>
      </div>
      <div style={{ display: 'flex', gap: '12px', 'font-size': '11px', ...mono }}>
        <span style={{ color: '#64748b' }}>
          PM2.5 <span style={{ color: pmT()?.color || '#94a3b8', 'font-weight': '600' }}>{pm()}</span>
        </span>
        <span style={{ color: '#64748b' }}>
          CO₂ <span style={{ color: '#94a3b8', 'font-weight': '600' }}>{co()}</span>
        </span>
        <Show when={props.entry.data.voc}>
          <span style={{ color: '#64748b' }}>
            VOC{' '}
            <span style={{ color: '#94a3b8', 'font-weight': '600' }}>
              {Number.parseFloat(props.entry.data.voc).toFixed(0)}
            </span>
          </span>
        </Show>
        <Show when={props.entry.data.humidity}>
          <span style={{ color: '#64748b' }}>
            RH{' '}
            <span style={{ color: '#94a3b8', 'font-weight': '600' }}>
              {Number.parseFloat(props.entry.data.humidity).toFixed(0)}%
            </span>
          </span>
        </Show>
      </div>
