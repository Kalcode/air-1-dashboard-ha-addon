...
export function getThreshold(type: string, value: string | undefined): ThresholdResult | null {
...
export function parseESPHome(text: string): Partial<SensorData> | null {
  ...
  return Object.keys(result).length > 0 ? (result as Partial<SensorData>) : null;
}
...
export function getRssiInfo(rssi: string): RssiInfo | null {
  const v = Number.parseFloat(rssi);
  if (Number.isNaN(v)) return null;
