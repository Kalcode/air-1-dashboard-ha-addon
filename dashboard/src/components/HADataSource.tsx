		...
		try {
			...
			if (data.devices && Array.isArray(data.devices)) {
				...
				if (sensorList.length === 1) {
					...
					const exists = sensorList.find(
						(s) => s.entity_id === selectedSensor(),
					);
	...
	return (
		<div class="ha-data-source">
			...
			<Show when={!loading() && !error() && sensors().length === 0}>
				...
				>
					No air quality sensors found. Make sure your Apollo AIR-1 sensors are configured in Home Assistant with the prefix "
					{config()?.sensor_prefix || "air1"}
					".
				</div>
			</Show>
				...
				>
					<Show when={sensors().length > 1}>
						<div style={{ flex: 1 }}>
							<label
								for="sensor-select"
								style={{ display: "block", "margin-bottom": "0.5rem", color: "#cbd5e1" }}
							>
								Select Sensor:
							</label>
							<select
